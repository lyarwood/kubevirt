...
func (c *Controller) sync(vm *virtv1.VirtualMachine, vmi *virtv1.VirtualMachineInstance, key string) (*virtv1.VirtualMachine, common.SyncError, error) {
	...
	case virtv1.Expand:
		expandVMCopy := vm.DeepCopy()
		if err = c.instancetypeMethods.ApplyToVM(expandVMCopy); err != nil {
			return vm, common.NewSyncError(fmt.Errorf("error encountered while expanding instance type into VirtualMachine: %v", err), common.FailedCreateVirtualMachineReason), nil
		}
		// TODO(lyarwood): Fold this into ApplyToVM?
		expandVMCopy.Spec.Instancetype = nil
		expandVMCopy.Spec.Preference = nil
		...
		if !equality.Semantic.DeepEqual(vm.Spec, expandVMCopy.Spec) {
			...
			if err != nil {
				return vm, common.NewSyncError(fmt.Errorf("error encountered when trying to update VirtualMachine with expanded instance type and preference: %v", err), FailedUpdateErrorReason), nil
			}
			return updatedVm, nil, nil
		}
	}
