...
func (p *Processor) Process(template *templatev1alpha1.VirtualMachineTemplate) field.ErrorList {
	...
	paramMap := make(map[string]templatev1alpha1.Parameter)
	...
	template.Spec.Message, _ = p.EvaluateParameterSubstitution(paramMap, template.Spec.Message)

	rawVM := template.Spec.VirtualMachine
	var decodedVM runtime.Object
	var err error
	if len(rawVM.Raw) > 0 {
		decodedVM, err = runtime.Decode(unstructured.UnstructuredJSONScheme, rawVM.Raw)
		if err != nil {
			templateErrors = append(templateErrors, field.Invalid(field.NewPath("VirtualMachine"), rawVM, fmt.Sprintf("unable to handle object: %v", err)))
		}
	} else {
		decodedVM = rawVM.Object.DeepCopyObject()
	}
	...
	stripNamespace(decodedVM)

	paramVM, err := p.SubstituteParameters(paramMap, decodedVM)
	if err != nil {
		templateErrors = append(templateErrors, field.Invalid(field.NewPath("VirtualMachine"), template.Spec.Parameters, err.Error()))
	}
	...
	gvk := decodedVM.GetObjectKind().GroupVersionKind()
	paramVM.GetObjectKind().SetGroupVersionKind(gvk)
	template.Spec.VirtualMachine = runtime.RawExtension{Object: paramVM}
...
// based on its name.
func GetParameterByName(t *templatev1alpha1.VirtualMachineTemplate, name string) *templatev1alpha1.Parameter {
	for i, param := range t.Spec.Parameters {
		if param.Name == name {
			return &(t.Spec.Parameters[i])
		}
	}
	return nil
}
...
func (p *Processor) EvaluateParameterSubstitution(params map[string]templatev1alpha1.Parameter, in string) (string, bool) {
	out := in
...
//   - ${PARAMETER_NAME}
func (p *Processor) SubstituteParameters(params map[string]templatev1alpha1.Parameter, item runtime.Object) (runtime.Object, error) {
...
func (p *Processor) GenerateParameterValues(t *templatev1alpha1.VirtualMachineTemplate) field.ErrorList {
	var errs field.ErrorList

	for i := range t.Spec.Parameters {
		param := &t.Spec.Parameters[i]
